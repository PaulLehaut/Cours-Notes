...
Pour une classification à K classes où chaque classe c est représentée par une vecteur one-hot: $y_i\in \R^k$ ($y_{i,c}=1 \ \ \text{si ce est la classe correcte pour }x_i, \ \ 0 \text{sinon}$), le réseau prédit un vectuer de probabilité $\tilde{y_{i}}=p_\theta(y|x_i)$ obtenu par une couche softmax (décrite juste après). La perte d'entropie croisée s'écrit alors:
