...
\sskip 
Soit un état $x$, on dit que $C_x$ est fermée si $C_x=\{y\in E| x\leftarrow y\}$. $x$ est un état absorbant si $C_x=\{x\}$ $C_x$ est fermée.
\bskip 
...
\sskip 
On pose $T^x = \inf_\{n\ge 1| X_n =x\}$. $x$ est dit transient si $\Prob(T^x=\pinf)>0$, il est dit récurrent si $\Prob(T^x=\pinf)=0$.
\bskip 
